...
router.post("/add", authenticate, authorizeRoles("user", "admin", "admin"), addAdmissionWish);
// router.get("/max/:uId", getHighestPriorityAdmissionWishByUID);
router.get("/getAll/:uId", authenticate, authorizeRoles("user", "admin"), getAllWishesByUID);
//router.get("/getUniqueEmails", getAllUniqueEmails);
router.get("/getByStatus/:status", authenticate, authorizeRoles("admin", "admin"), getWishesByStatus);
